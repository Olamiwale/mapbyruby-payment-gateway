...
class PaymentService {
    ...
    static async verifyPaystack(reference) {
        try {
            const response = await axios_1.default.get(`${payment_1.paystackConfig.baseURL}/transaction/verify/${reference}`, { headers: { Authorization: `Bearer ${payment_1.paystackConfig.secretKey}` } });
            ...
            await database_1.default.$transaction([
                database_1.default.payment.update({ where: { id: payment.id }, data: { status } }),
            ...
            if (status === 'SUCCESS') {
                email_1.EmailService.sendOrderConfirmation(payment.user.email, payment.user.firstName, {
            ...
            logger_1.Logger.info('Paystack payment verified', { paymentId: payment.id, status });
