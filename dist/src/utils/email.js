...
class EmailService {
    // ── Welcome Email ─────────────────────────────────────────
    static async sendWelcome(to, firstName) {
        try {
          ...
          <body style="font-family: Georgia, serif; background: #f5f4f0; margin: 0; padding: 40px 16px;">
            <div style="max-width: 560px; margin: 0 auto; background: #fff; border-radius: 12px; overflow: hidden; border: 1px solid #e8e6e0;">
              ...
              <div style="padding: 40px 36px;">
                ...
                <a href="${env_1.env.FRONTEND_URL}/product"
        ...
        `,
            ...
            logger_1.Logger.info('Welcome email sent', { to });
        }
        catch (error) {
            logger_1.Logger.error('Failed to send welcome email', { to, error });
            // Never throw — email failure should not break registration
        }
    }
    // ── Order Confirmation Email ───────────────────────────────
    static async sendOrderConfirmation(to, firstName, order) {
        try {
          ...
          <body style="font-family: Georgia, serif; background: #f5f4f0; margin: 0; padding: 40px 16px;">
            <div style="max-width: 560px; margin: 0 auto; background: #fff; border-radius: 12px; overflow: hidden; border: 1px solid #e8e6e0;">
              ...
              <div style="padding: 40px 36px;">
                ...
                <a href="${env_1.env.FRONTEND_URL}/profile"
                   style="display: inline-block; background: #1a1a1a; color: #fff; text-decoration: none;
        ...
        `,
            ...
            logger_1.Logger.info('Order confirmation email sent', { to, reference: order.reference });
        }
        catch (error) {
            logger_1.Logger.error('Failed to send order confirmation email', { to, error });
        }
    }
    // ── Password Reset Email ───────────────────────────────────
    static async sendPasswordReset(to, firstName, resetToken) {
        try {
        ...
        `,
            ...
            logger_1.Logger.info('Password reset email sent', { to });
        }
        catch (error) {
            logger_1.Logger.error('Failed to send password reset email', { to, error });
        }
    }
}
